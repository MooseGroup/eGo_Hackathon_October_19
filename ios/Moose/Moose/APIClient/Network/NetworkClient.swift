...
protocol NetworkClientRequestable {
    @discardableResult
    func request(_ request: URLRequest, completion: @escaping (Result<NetworkClient.Response>) -> Void) -> URLSessionDataTask?
}
...
class NetworkClient: NSObject, URLSessionTaskDelegate, NetworkClientRequestable {
    ...
    func request(_ request: URLRequest, completion: @escaping (Result<Response>) -> Void) -> URLSessionDataTask? {
        print("Request: \(request.httpMethod) \(request) \(String(describing: request.allHTTPHeaderFields))")
