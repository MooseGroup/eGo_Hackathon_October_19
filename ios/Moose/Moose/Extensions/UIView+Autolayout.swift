...
public extension AutoLayoutContainer {
    ...
    // MARK: Leading
    @discardableResult
    func pinToLeading(of container: AutoLayoutContainer, inset: CGFloat = 0) -> NSLayoutConstraint {
        if let view = self as? UIView {
            view.translatesAutoresizingMaskIntoConstraints = false
        }
        let constraint = leadingAnchor.constraint(equalTo: container.leadingAnchor, constant: inset)
    ...
    // MARK: Trailing
    @discardableResult
    func pinToTrailing(of container: AutoLayoutContainer, inset: CGFloat = 0) -> NSLayoutConstraint {
        if let view = self as? UIView {
            view.translatesAutoresizingMaskIntoConstraints = false
        }
        let constraint = trailingAnchor.constraint(equalTo: container.trailingAnchor, constant: -inset)
        constraint.isActive = true
        return constraint
    }
